...
(function () {
  var KEYWORDS = "abstract break catch char class " +
    "continue default do double else enum false float for if int long new " +
    "private protected public return short static switch this throw true try " +
    "void volatile while byte finally interface null assert " +
    ...
    if (style === PR_PLAIN) {
      ...
      for (var j = 0, m = subDecs.length; j < m; j += 2) {
        ...
        if (subStyle === PR_PLAIN) {
          ...
          } else if (/^@?[A-Z][A-Z$]*[a-z][A-Za-z0-9$]*$/.test(token)) {
...
var PR_MARKUP_LEXER = PR_createSimpleLexer([], [
    ...
    [PR_SOURCE,
     ...
     /^<(script|style)\b[^>]*>[\s\S]*?<\/\1\b[^>]*>/i, null],
    [PR_TAG,         /^<\/?\w[^<>]*>/, null]
    ]);
// Splits any of the source|style entries above into a start tag,
...
/**
 * Find all the < code > tags in the DOM and prettify them.
 */
function prettyPrint()
{
  ...
  function doWork() {
    var endTime = (PR_SHOULD_USE_CONTINUATION ? new Date().getTime() + 250 : Infinity);
