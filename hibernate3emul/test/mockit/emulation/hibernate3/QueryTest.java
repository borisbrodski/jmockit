...
public final class QueryTest
{
   ...
   public void findChildrenEntitiesWithGivenParentName()
   {
      addChildrenEntities();
      QueryEmul query =
         new QueryEmul("select c from ChildEntity c where c.parent.name like ?", entities);
   ...
   public void findChildrenAndParentEntitiesWithGivenParentSeqNumber()
   {
      ...
      QueryEmul query =
         new QueryEmul(
            "select c, p from ChildEntity c join c.parent p where p.seqNumber=3", entities);

      List<Object[]> found = (List<Object[]>) query.list();

      assertEquals(1, found.size());
      Object[] result = found.get(0);
